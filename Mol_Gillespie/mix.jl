...
function molecular_simulation_and_Gillespie(initial_state,react_left,react_right,react_rate,EP_index,distance_affect_rate,  t_max,dt,part_num,mass,initial_pos,initial_vel,force_types,iterate_method,boundary,frame_interval)
    ...
    f_temp=zeros(3)
    f_temp2=zeros(part_num,3)
    other_para=0
    frame_record=zeros(part_num,3,1+ceil(Int64,t_max/(frame_interval*dt))) #用于收集帧
    count_loop=0
    count_frame=0
    ...
    if iterate_method=="leap-frog verlet" #预估蛙跳verlet算法初速度
        @. vel=vel-force*dt/mass/2
    elseif iterate_method=="langevin leap-frog" #计算langevin leap-frog算法迭代起始速度
        f_temp2.=randn(part_num,3)
        @. vel=vel*exp(-γ*dt/2)+(1-exp(-γ*dt/2))/γ*force/mass+sqrt(kB*T*(1-exp(-γ*dt)))*f_temp2/sqrt(mass)
        alpha=exp(-γ*dt)
        other_para=alpha
    end
    
    #主循环
    while count_loop*dt<t_max 
        #采样
        if count_loop%frame_interval==0
            count_frame+=1
        ...
        #分子动力学
        #更新位置，速度，力
        step_update!(part_num,mass,pos,vel,force_types,force,dt,boundary,f_temp,iterate_method,other_para,f_temp2)
            
        #化学反应
        if count_loop*dt>=t[end]
            #f_mu=反应常数+距离影响
    ...
    return (pos,state,frame_record,state_record,t)
end