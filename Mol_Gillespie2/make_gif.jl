...
function make_gif(frame_rec::Array{Float64, 3},path,fps,EP_index=nothing,title="example",label="data")
    ani=Animation()
    ...
    for i in 1:size(frame_rec,3)
        ...
        if EP_index!=nothing
            scatter!(frame_rec[EP_index,1,i],frame_rec[EP_index,2,i],frame_rec[EP_index,3,i],label="EP",mc=:blue,ms=5,ma=1)
        end
        frame(ani,p)
    end
